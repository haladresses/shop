...
const SingleItem = ({ item, removeItemFromCart }) => {
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      {/* Image + text — dir="rtl" from parent handles order */}
      <div className="flex-1 flex items-center gap-4 min-w-0">
        <div className="flex-shrink-0 w-[80px] h-[80px] rounded-[10px] bg-gray-3 overflow-hidden">
          <Image
        ...
        <div className="min-w-0">
          ...
          {variantLabel && (
            <p className="text-custom-xs text-dark-4 mb-1 truncate">{variantLabel}</p>
          )}
          <p className="text-custom-sm text-dark-4">
            {isArabic ? "السعر:" : "Price:"}{" "}
            <span className="text-dark font-medium">
              {Number(item.discountedPrice).toFixed(3)} {currency}
            </span>
          </p>
        </div>
      </div>
      ...
      <button
        ...
        className="flex-shrink-0 flex items-center justify-center rounded-lg w-9 h-9 bg-gray-2 border border-gray-3 text-dark ease-out duration-200 hover:bg-red-light-6 hover:border-red-light-4 hover:text-red"
      >
