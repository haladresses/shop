...
const PLACEHOLDER_IMAGE = "/images/products/p1.png";

const galleryOf = (p: { imgs?: { previews?: string[] } }) =>
  p?.imgs?.previews && p.imgs.previews.length > 0 ? p.imgs.previews : [PLACEHOLDER_IMAGE];
...
const QuickViewModal = () => {
  ...
  // The product-list cards only carry the primary image (kept light for grid
  // performance) — the quick-view gallery fetches the full image set by slug
  // once it opens, then upgrades from that single placeholder to the real set.
  const [previews, setPreviews] = useState<string[]>(galleryOf(product));
  ...
  const goToPreview = (key: number) => {
    setActivePreview(key);
    sliderRef.current?.swiper.slideTo(key);
  };

  // reset the gallery to the first image whenever a different product opens,
  // then fetch that product's full image set (list cards only carry one)
  useEffect(() => {
    setActivePreview(0);
    setPreviews(galleryOf(product));
    ...
    fetchProductBySlug(product.slug)
      .then((apiProduct) => {
        if (cancelled || !apiProduct) return;
        const full = mapApiProduct(apiProduct, isArabic ? "ar" : "en");
        if (full.imgs.previews.length > 0) setPreviews(full.imgs.previews);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
          ...
          <div className="flex flex-wrap items-center gap-12.5">
            <div className="max-w-[526px] w-full">
              <div className="flex gap-5">
                <div className="flex flex-col gap-5 max-h-[508px] overflow-y-auto no-scrollbar">
                  {previews.map((img, key) => (
                    <button
                      onClick={() => goToPreview(key)}
                      key={key}
                      aria-label={`show image ${key + 1}`}
                      className={`relative flex items-center justify-center w-20 h-20 shrink-0 overflow-hidden rounded-lg bg-gray-1 ease-out duration-200 hover:border-2 hover:border-blue ${activePreview === key && "border-2 border-blue"
                        }`}
                    >
                      <Image
                        src={img || ""}
                ...
                <div className="quickview-gallery relative z-1 overflow-hidden w-full sm:min-h-[508px] bg-gray-1 rounded-lg border border-gray-3 [--swiper-pagination-color:#8C3B49]">
                  <button
                  ...
                  <Swiper
                    ref={sliderRef}
                    modules={[Pagination]}
                    pagination={previews.length > 1 ? { clickable: true } : false}
                    onSlideChange={(swiper) => setActivePreview(swiper.activeIndex)}
                    className="w-full h-full sm:min-h-[508px]"
                  >
                    {previews.map((src, key) => (
                      <SwiperSlide key={key}>
                        <div className="relative w-full h-[320px] sm:h-[508px]">
                          <Image
                            src={src}
                            alt={`${product.title || "product"} image ${key + 1}`}
                            fill
                  ...
                  {previews.length > 1 && (
                    <>
                      <button
                        ...
                        className="absolute left-3 top-1/2 -translate-y-1/2 z-40 flex items-center justify-center w-9 h-9 rounded-full bg-white/90 shadow-1 text-dark ease-out duration-200 hover:text-blue hover:bg-white"
                      >
                        <svg className="fill-current" width="18" height="18" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path fillRule="evenodd" clipRule="evenodd" d="M14.5918 5.92548C14.9091 5.60817 15.4236 5.60817 15.7409 5.92548L22.2409 12.4255C22.5582 12.7428 22.5582 13.2572 22.2409 13.5745L15.7409 20.0745C15.4236 20.3918 14.9091 20.3918 14.5918 20.0745C14.2745 19.7572 14.2745 19.2428 14.5918 18.9255L19.7048 13.8125H4.33301C3.88428 13.8125 3.52051 13.4487 3.52051 13C3.52051 12.5513 3.88428 12.1875 4.33301 12.1875H19.7048L14.5918 7.07452C14.2745 6.75722 14.2745 6.24278 14.5918 5.92548Z" fill="" />
                        </svg>
                      </button>
                      <button
                        onClick={handleNext}
                        aria-label="next image"
                        className="absolute right-3 top-1/2 -translate-y-1/2 z-40 flex items-center justify-center w-9 h-9 rounded-full bg-white/90 shadow-1 text-dark ease-out duration-200 hover:text-blue hover:bg-white rotate-180"
                      >
                        <svg className="fill-current" width="18" height="18" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path fillRule="evenodd" clipRule="evenodd" d="M14.5918 5.92548C14.9091 5.60817 15.4236 5.60817 15.7409 5.92548L22.2409 12.4255C22.5582 12.7428 22.5582 13.2572 22.2409 13.5745L15.7409 20.0745C15.4236 20.3918 14.9091 20.3918 14.5918 20.0745C14.2745 19.7572 14.2745 19.2428 14.5918 18.9255L19.7048 13.8125H4.33301C3.88428 13.8125 3.52051 13.4487 3.52051 13C3.52051 12.5513 3.88428 12.1875 4.33301 12.1875H19.7048L14.5918 7.07452C14.2745 6.75722 14.2745 6.24278 14.5918 5.92548Z" fill="" />
                        </svg>
